...
def create_video(
    payload: VideoCreateRequest, user: CurrentUser, db: DbSession
) -> VideoCreateResponse:
    ...
    # claude.md M2: the default American stock voice is used before the profile has its
    # own clone, so the pipeline is testable end-to-end without requiring a voice clone.
    ...
    return VideoCreateResponse(video_request_id=video_request.id, job_id=job.id)
